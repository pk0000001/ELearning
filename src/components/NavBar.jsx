    ...
    >
      ...
      {!isLoggedIn ? (
        ...
        >
          ...
          <button className="px-4 py-2 border border-black rounded-md hover:bg-blue-800 hover:text-white transition" onClick={handleLogout}>Logout</button>
        )}
      </div>
      ...
      {menuOpen && (
        <div className="absolute top-16 left-0 w-full bg-white shadow-md flex flex-col items-center sm:hidden">
          ...
          <button className="w-full py-3 border-b text-black hover:bg-blue-800 hover:text-white" onClick={handleLogout}>Logout</button>
        )}
        </div>
      )}
